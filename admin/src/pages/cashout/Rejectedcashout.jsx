...
const Rejectedcashout = () => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        const response = await axios.get(`${base_url}/auth/cash-out/rejected`);
...
const updateStatus = async (id, newStatus) => {
  try {
    const response = await axios.put(`${base_url}/auth/cash-out/${id}`, { status: newStatus });
...
// Update the deleteTransaction function to include toast notifications
const deleteTransaction = async (id) => {
  if (window.confirm('Are you sure you want to delete this transaction?')) {
    try {
      await axios.delete(`${base_url}/auth/cash-out/${id}`);
      ...
      <div className="flex pt-[10vh] h-[90vh]">
        ...
        >
          <div className="bg-white  py-4 px-2">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Rejected Cash Out Transactions</h2>
...
export default Rejectedcashout;